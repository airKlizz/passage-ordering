...
# Lint as: python3
"""CNN/DailyMail Ordering dataset, non-anonymized version."""
...
_DESCRIPTION = """\
CNN/DailyMail non-anonymized ordering dataset.
"""
...
class CnnDailymail(datasets.GeneratorBasedBuilder):
    ...
    def _generate_examples(self, files):
        for p in files:
            sentences = _get_art_abs(p)
            if sentences == []:
                continue
            fname = os.path.basename(p)
            shuffled_sentences, label = self.shuffle_sentences(sentences)
            """ Only keep reasonable examples """
            if len(label) > 20:
                continue
            if len(" ".join(sentences).split()) > 1024:
                continue
            if max(label) >= 128:
                continue
            yield fname, {
