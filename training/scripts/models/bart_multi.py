...
class BartForSequenceOrderingWithMultiPointer(PretrainedBartModel, OrderingMixin):
    ...
    def forward(
        ...
        heads_logits_permute = heads_logits.permute(0, 2, 3, 1)
        logits = self.heads_combination(heads_logits_permute).squeeze(-1)
        heads_logits = heads_logits.transpose(3, 2)
        ...
        raw_heads_logits = heads_logits.masked_fill(sequence_attention_mask.unsqueeze(1) == 0, float("-inf"))
        heads_logits = raw_heads_logits * self.heads_combination.weight.unsqueeze(-1).unsqueeze(-1)
        heads_logits[heads_logits == float("inf")] = float("-inf")
